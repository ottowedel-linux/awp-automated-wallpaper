...
# ---------- backend Generic (Openbox + Feh) ----------
def generic_force_single_workspace_off():
    pass

def generic_set_wallpaper(ws_num, image_path, scaling):
...
def universal_confirm_deletion(wallpaper_name):
    """Qt-based confirmation dialog"""
    if HAS_QT:
        try:
...
def delete_current_wallpaper_and_advance():
    """Delete current wallpaper and advance to next one - matching index behavior of next/prev"""
    ws_num = get_current_workspace()
    ws_key = get_ws_key(ws_num)
    
    # Load config and state (EXACTLY same as next/prev)
    cfg = load_config()
    state = load_state()
    sec = f"ws{ws_num+1}"
    ...
    # Get current images and index (EXACTLY same as next/prev)
    ...
    # RELOAD images (without deleted one) and RE-SORT (critical!)
    imgs = load_images(folder)
    if not imgs:
        logging.error("No wallpapers left after deletion")
        # Handle empty folder case - maybe set a default?
        return True
    ...
    # Calculate new index (EXACTLY same logic as "next" command)
    if mode == 'random':
        if len(imgs) == 1:
            new_idx = 0
        else:
